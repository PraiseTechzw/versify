...
const ImageUploader = memo(function ImageUploader({ onImageUpload, currentImage }: ImageUploaderProps) {
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      console.log('✅ File type valid, starting processing...')
      ...
      try {
          ...
          dataUrl = await new Promise<string>((resolve, reject) => {
            ...
            reader.onprogress = (e) => {
              if (e.lengthComputable) {
                const progress = 25 + Math.round((e.loaded / e.total) * 50)
        ...
        console.log('🎉 Upload successful, calling onImageUpload')
  ...
  const handlePlaceholderSelect = useCallback(
    (image: ImagePlaceholder) => {
      console.log('🖼️ Gallery image selected:', image.description)
  ...
  const triggerFileInput = useCallback(() => {
    console.log('🖱️ Upload area clicked, currentImage:', currentImage)
    if (!currentImage) {
      console.log('✅ Triggering file input click')
      fileInputRef.current?.click()
    } else {
      console.log('⚠️ Image already exists, not triggering file input')
    }
  }, [currentImage])
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Dialog open={isCameraOpen} onOpenChange={(open) => {
          console.log('📷 Camera dialog state changing to:', open)
      ...
      <Dialog open={isGalleryOpen} onOpenChange={(open) => {
        console.log('🖼️ Gallery dialog state changing to:', open)
        ...
        <DialogTrigger asChild>
          <Button 
            ...
            onClick={() => console.log('🖼️ Gallery button clicked')}
          >
            ...
            {PlaceHolderImages.map((image) => (
              <button
                ...
                onClick={(e) => {
                  e.preventDefault()
                  e.stopPropagation()
                  console.log('🖼️ Gallery item clicked:', image.id)
                  handlePlaceholderSelect(image)
                }}
